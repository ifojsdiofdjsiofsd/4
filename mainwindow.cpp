...
void MainWindow::onNetworkConnected()
{
    ...
    if (m_isMyTurn) {
        // Если мы сервер, отправляем состояние поля
        sendBoardState();
    } else {
        // Если мы клиент, запрашиваем состояние поля
        m_networkManager->sendData("REQUEST_BOARD");
    }

    m_faceButton->setText(m_isMyTurn ? "▶" : "⏸");
}

void MainWindow::sendBoardState()
{
    if (m_networkManager->isConnected()) {
        QByteArray boardData = serializeBoardState();
        m_networkManager->sendData("BOARD_STATE " + boardData);
    }
}
...
void MainWindow::onNetworkDataReceived(const QByteArray &data)
{
    ...
    if (msg.startsWith("BOARD_STATE ")) {
        // Получено состояние поля
        QByteArray boardData = msg.mid(12).toUtf8();
        receiveBoardState(boardData);
    }
    else if (msg == "REQUEST_BOARD") {
        // Запрос состояния поля (от клиента)
        sendBoardState();
    }
    else if (msg.startsWith("MOVE ")) {
        // Обработка хода (как было раньше)
        ...
        if (parts.size() == 3) {
            ...
            m_isMyTurn = true;
            m_faceButton->setText("▶");
        }
    }
}

void MainWindow::receiveBoardState(const QByteArray &data)
{
    // Временно останавливаем таймер, чтобы не обновлять время
    bool timerWasRunning = m_timer->isActive();
    m_timer->stop();

    // Сохраняем текущее время
    int savedTime = m_timeElapsed;

    // Десериализуем состояние поля
    deserializeBoardState(data);

    // Восстанавливаем состояние таймера
    m_timeElapsed = savedTime;
    ...
    updateMineCounter(m_mines - m_gameBoard->flagsUsed());
}
...
void MainWindow::cellRightClick(int row, int col)
{
    ...
    if (m_isNetworkGame) {
        // Отправляем обновленное состояние поля при установке/снятии флага
        sendBoardState();
    }
}
...
QByteArray MainWindow::serializeBoardState() const
{
    QByteArray data;
    QDataStream stream(&data, QIODevice::WriteOnly);

    // Записываем размеры поля и количество мин
    stream << m_rows << m_cols << m_mines;

    // Записываем состояние каждой клетки
    for (int row = 0; row < m_rows; ++row) {
        for (int col = 0; col < m_cols; ++col) {
            const Cell &cell = m_gameBoard->getCell(row, col);
            stream << cell.hasMine << cell.isRevealed << cell.isFlagged;
        }
    }
...
void MainWindow::deserializeBoardState(const QByteArray &data)
{
    QDataStream stream(data);
    int rows, cols, mines;
    stream >> rows >> cols >> mines;

    // Если размеры не совпадают, создаем новое поле
    if (rows != m_rows || cols != m_cols || mines != m_mines) {
        m_rows = rows;
        m_cols = cols;
        m_mines = mines;
        changeMode(0); // Сброс режима (реальный режим будет установлен ниже)
    }

    // Временно отключаем сигналы, чтобы избежать лишних обновлений
    m_gameBoard->blockSignals(true);

    // Восстанавливаем состояние клеток
    for (int row = 0; row < m_rows; ++row) {
        for (int col = 0; col < m_cols; ++col) {
            bool hasMine, isRevealed, isFlagged;
            stream >> hasMine >> isRevealed >> isFlagged;

            Cell &cell = const_cast<Cell&>(m_gameBoard->getCell(row, col));
    ...
    m_gameBoard->blockSignals(false);
    updateMineCounter(m_mines - m_gameBoard->flagsUsed());
}
